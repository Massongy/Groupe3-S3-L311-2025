window.addEventListener("DOMContentLoaded", (event) => {
    let is_run = true;  // changer en booléen, pas string

    init();

    function init(){
        document.querySelector('.button').addEventListener('click', (event) => { //ajout de l'accolade
            is_run = !is_run;
            ((' ' + document.querySelector('.button').className + ' ').indexOf('pause') > -1) 
            // modification de dcment en document
...
//modification de setTimeInterval en setInterval
        ...
        }, 1000); //ajout de la parenthèse fermante
    }

    function adjustTimer(timer){
        return (timer < 10 ? '0'+timer : timer);
    }
// ajout du paramètre z
